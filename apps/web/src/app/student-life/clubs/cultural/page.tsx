...
const get = async <T,>(query: string, params?: QueryParams): Promise<T> => {
  try {
    ...
    return [] as T; // Return an empty array on error
  }
};

async function getCulturalClubs(): Promise<Club[]> {
  const query = `*[_type == "culturalClub"]{
    _id,
    name,
    description,
    logo,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {organizations.map((org) => (
              <ClubCard
                ...
                imageUrl={org.image}
                members={org.members}
                meetingDetails={org.meetingDetails}
                links={org.links}
                vision={org.vision}
                mission={org.mission}
              />
